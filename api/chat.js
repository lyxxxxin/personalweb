...
const TOOLS = [
  {
    ...
    description: '获取在美团的工作经历详情，包括职责、参与的技术方向、使用的技术栈。',
    input_schema: { type: 'object', properties: {} },
  },
  {
    name: 'get_project_details',
    description: '获取某个具体项目的详细介绍，包括背景、具体工作内容、技术选型、性能指标。',
    input_schema: {
      type: 'object',
      properties: {
        project: {
          ...
          description:
            'vector_index = 向量索引稳定性与召回优化；suggest_stability = Suggest 稳定性建设；hot_list = 闪购热榜建设',
        },
      },
      required: ['project'],
    },
...
function executeTool(name, input) {
  switch (name) {
    ...
    case 'get_project_details':
      return DATA.projects[input.project] || { error: 'Project not found' };
    case 'get_skills': {
      const cat = input.category || 'all';
      if (cat === 'all') return DATA.skills;
      return { [cat]: DATA.skills[cat] };
    }
    case 'get_education':
...
- 回答简洁，简单问题 2-3 句，复杂问题不超过 8 句；适当使用换行让内容清晰
- 如有性能指标（如 QPS、延迟），要具体引用数字，这是亮点
...
module.exports = async function handler(req, res) {
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    return res.status(200).json({
      response: '⚙️ Agent 暂未配置 API Key，请在 Vercel 项目设置 → Environment Variables 中添加 ANTHROPIC_API_KEY，重新部署后即可使用。',
    });
  }
  ...
  try {
    ...
    // Agent loop: keep running until end_turn (handles multi-step tool use)
    for (let i = 0; i < 10; i++) {
      ...
      if (response.stop_reason === 'end_turn') {
        ...
        return res.status(200).json({ response: text?.text || '' });
      }
